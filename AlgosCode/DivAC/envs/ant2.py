...
def rate_buffer(buffer):
    visited_blocks = [get_state_block(state) for state in buffer]
...
class AntEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def step(self, action):
        ...
        xposafter = np.copy(self.get_body_com("torso")[0:1])
        yposafter = np.copy(self.get_body_com("torso")[1:2])

        forward_reward = (xposafter - self.prev_x_torso) / self.dt
        # forward_reward = (xposafter - self.prev_x_torso) / self.dt + (yposafter - self.prev_y_torso) / self.dt
        ctrl_cost = .005 * np.square(action).sum()
        survive_reward = 1.0
