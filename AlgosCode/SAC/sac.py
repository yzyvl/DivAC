...
def sac(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0,
        ...
        polyak=0.995, lr=1e-3, alpha=0.05, batch_size=256, start_steps=10000,
        max_ep_len=1000, save_freq=1, filename=None):
    """
    ...
    # Count variables
    var_counts = tuple(core.count_vars(scope) for scope in ['main/pi', 'main/q1', 'main/q2', 'main/v', 'main'])
    ...
    for t in range(total_steps):
        ...
        if t > 0 and t % steps_per_epoch == 0:
            ...
            evalutions.append(test_reward)
            print('Epoch:', epoch, "rewards:", test_reward)
...
if __name__ == '__main__':
    ...
    filename = os.path.join(args.log, "%s_%s_%s" % (args.env, args.exp_name, args.seed))

    sac(lambda: gym.make(args.env), actor_critic=core.mlp_actor_critic,
        ac_kwargs=dict(hidden_sizes=[args.hid] * args.l),
        gamma=args.gamma, seed=args.seed, epochs=args.epochs, filename=filename)
